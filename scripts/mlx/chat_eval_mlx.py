"""
Evaluate the Chat model on Apple Silicon using MLX.
All the generic code lives here, and all the evaluation-specific
code lives in nanochat directory and is imported from here.

Example runs:
python -m scripts.chat_eval_mlx -a ARC-Easy
python -m scripts.chat_eval_mlx -a MMLU
"""
...
def run_generative_eval(task_object, tokenizer, model, engine, num_samples, max_new_tokens, temperature, top_k, max_problems=None):
    ...
    # Run the evaluation (no distributed training in MLX)
...
def run_categorical_eval(task_object, tokenizer, model, batch_size, max_problems=None):
    ...
    for i in range(num_batches):
        ...
        prompt_ids = mx.array(padded_prompt_ids, dtype=mx.int32)
        ...
        for idx, conversation in enumerate(conversations):
            ...
            argmax_letter_id = int(mx.argmax(focus_logits))
