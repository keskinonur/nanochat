"""
Evaluate the CORE metric for a given model on Apple Silicon using MLX.

Run on Apple Silicon:
python -m scripts.base_eval_mlx
...
from nanochat.core_eval_mlx import evaluate_task
...
def load_hf_model(hf_path: str, device):
    ...
    # Note: HuggingFace models loaded here still use PyTorch/CUDA, not MLX
...
# -----------------------------------------------------------------------------
def main():
    assert len(sys.argv) in [1, 2], "Usage: python -m scripts.base_eval_mlx [hf_path]"

    # distributed / precision setup
    ddp, ddp_rank, ddp_local_rank, ddp_world_size, device = compute_init()
    # MLX doesn't need autocast context (automatic mixed precision)
    ...
    # Evaluate the model
    # MLX doesn't need autocast context
    out = evaluate_model(model, tokenizer, device)

    # Write out the results to a csv file
    core_metric = None
    centered_results = {}
    # MLX doesn't have distributed training, so always write results
    base_dir = get_base_dir()
