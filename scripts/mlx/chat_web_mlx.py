#!/usr/bin/env python3
"""
NanoChat Web Server for MLX models on Apple Silicon.
Serves both UI and API from a single FastAPI instance.

Run with: python -m scripts.chat_web_mlx -i sft
...
parser = argparse.ArgumentParser(description='NanoChat Web Server (MLX)')
...
async def lifespan(app: FastAPI):
    ...
    print("Loading nanochat model (MLX)...")
    app.state.model, app.state.tokenizer, _ = load_model(
...
async def chat_completions(request: ChatRequest):
    ...
    if request.stream:
        ...
        response_tokens = result_tokens[0][len(conversation_tokens):]
...
if __name__ == "__main__":
    import uvicorn
    print(f"Starting NanoChat Web Server (MLX)")
